...
\end{tikzcd}
\end{figure}
the maps $p_x$ are called the \emph{projections}.
\end{definition}
...
So we have an isomorphism $F\ell \simeq \ell$. To show that it is initial we consider any $F$-algebra $(a, \alpha)$, and look at the data:
\begin{figure}[H]
\centering
\begin{tikzcd}
...
To show that there is a unique algebra homomorphism which we suggestively denote $\lbanana \alpah \rbanana$ from $\ell$ to $a$, we will first show that if it exists, then it should be the unique mediating arrow $\bar{f}$ between the cocones with vertices $\ell$ and $a$ respectively.
\end{proof}
